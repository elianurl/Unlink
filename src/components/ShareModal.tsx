...
import { X, Heart, Share2, Link2, Check, Github, Star } from "lucide-react";

const SHARE_URL = "https://unlink-data.vercel.app";
const GITHUB_URL = "https://github.com/elianurl/Unlink";
const SHARE_TEXT =
...
export default function ShareModal({ onClose, celebration = false }: ShareModalProps) {
  ...
  const handleInstagram = async () => {
    // Instagram no permite precargar enlaces: copiamos el texto para que el
    // usuario lo pegue en su historia y abrimos la cámara de stories en móvil.
    try {
      await navigator.clipboard.writeText(`${SHARE_TEXT} ${SHARE_URL}`);
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <a
              ...
              className="flex flex-col items-center gap-1.5 py-3 rounded-xl border text-xs font-medium transition-colors bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100"
            >
              <WhatsAppIcon />
              WhatsApp
            </a>
            <button
              ...
              className="flex flex-col items-center gap-1.5 py-3 rounded-xl border text-xs font-medium transition-colors bg-pink-50 text-pink-700 border-pink-100 hover:bg-pink-100"
            >
              <InstagramIcon />
              Instagram
            </button>
          ...
          {instagramHint && (
            <p className="text-center text-xs text-pink-600 font-medium bg-pink-50 border border-pink-100 rounded-lg py-2 px-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium transition-colors shadow-lg shadow-slate-900/15"
          >
            <Github size={17} />
            Valóranos en GitHub
          ...
          <p className="text-center text-xs text-slate-400 leading-relaxed pt-1">
            Unlink es gratuito, de código abierto y sin publicidad. Síguenos en GitHub para descubrir más proyectos que te pueden servir. 💜
          </p>
        </div>
      </motion.div>
    </div>
